...
test('Sign In - Past Due User', async ({ page }) => {
  test.setTimeout(60000);

  // Load test data from JSON file
  const testDataPath = path.join(__dirname, 'data', 'apply_test_data.json');
  const testData = JSON.parse(fs.readFileSync(testDataPath, 'utf-8'));
  const { email, password } = testData.loginAccounts.pastDue;
  ...
  await page.screenshot({ path: 'screenshots/sign_in_past-due.png', fullPage: true });
